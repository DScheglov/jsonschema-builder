...
function processModels(pathToModels, options) {
  ...
  try {
    ...
    if (processModels.consoleOutput) {
...
processModels.extendMongoose = jsonSchemaExtention;
...
function __getMongoose(model) {
  ...
  if (!mongoose || mongoose && mongoose.constructor.name !== 'Mongoose') {
...
function __id_mapper(id) {
  ...
  return helpers.formatId.bind(null, '-');;
}
